...
import { currentPlayer, getLegalTilePlacements } from '@babel-game/game-core';
...
function Board({ G, moves }: BoardProps<GameState>) {
  ...
  const options = G.drawnTile ? getLegalTilePlacements(G.board, G.drawnTile) : [];
  ...
  return (
    <div style={{ fontFamily: 'system-ui', padding: 24 }}>
      ...
      <p>
        Drawn tile: {G.drawnTile ? `${G.drawnTile.terrain}/${G.drawnTile.river}` : '—'}
      </p>
      <button
        disabled={!first}
        onClick={() => first && m.placeTile(first.at, first.rotations[0]!)}
      >
